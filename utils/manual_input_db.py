...
if __name__ == "__main__":
  ...
  data_type = "annual" # "annual" or "quarter"
  ...
  if (data_type == "annual"):
    ...
    print(f"[SUCCESS] Successfully insert {count} of {len(data_dict)} data to database")


  elif (data_type == "quarter"):
    df = pd.read_csv("data/data_quarter_2024_audit.csv")
    df = df.drop(['industry_code'], axis=1)
    df = df.replace({np.nan: None})
    data_dict = df.to_dict(orient="records")
    count = 0
