...
def process_dataframe(
    ...
    period_arg: str,
    year_arg: int,
    previous_period_data: pd.DataFrame = None,
    process: int = 1,
):
            ...
            for _, row in curr_symbol_df.iterrows():
                ...
                try:
                    ...
                    if previous_period_data is not None:
                        try:
                            previous_income_statement_entry = previous_period_data.loc[
                                row["symbol"], "income_stmt_metrics"
                            ]
                            previous_income_statement_data = json.loads(
                                previous_income_statement_entry
                            )
